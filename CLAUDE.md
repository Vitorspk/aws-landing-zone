...
- Any destructive `workflow_dispatch` (destroy/teardown) must require an explicit confirmation before any AWS calls run: prefer a `type: choice` input constrained to safe values (see `destroy-ingress-nginx.yml`'s `confirm` input) when the accepted values are limited; if you need a literal string like `DESTROY` typed exactly (see `destroy-infrastructure.yml`'s `confirm_destroy` input), it must be a `type: string` input whose value is routed through `env:` before use in any `run:` block — never interpolate a free-text input directly into a shell command via `${{ }}`.
